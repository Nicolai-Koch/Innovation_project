<script lang="ts">
  ...
  export let lightServices: JDService[] = [];
  ...
  $: if (selectedLightIdx >= lightServices.length) selectedLightIdx = 0;
  ...
  async function readLightValue(service: JDService): Promise<number> {
    const register =
      service.serviceClass === SRV_LIGHT_LEVEL
        ? service.register(LightLevelReg.LightLevel)
        : service.register(IlluminanceReg.Illuminance);
  ...
  async function tick() {
    ...
    try {
      const lightService = lightServices[selectedLightIdx];
      const ledService = ledServices[selectedLedIdx];
      if (!lightService || !ledService) {
        status = 'Missing light sensor or LED ring.';
        running = false;
        stopLoop();
        return;
      }

      latestReading = await readLightValue(lightService);

      if (latestReading >= threshold) {
        blinkOn = !blinkOn;
        await setLed(ledService, blinkOn);
        status = `Bright light detected (>= ${threshold}) - blinking`;
      } else {
        blinkOn = false;
  ...
  async function startAutomation() {
    if (running) return;
    if (!lightServices.length || !ledServices.length) {
      status = 'Connect one light sensor and one LED ring first.';
      return;
    }

    running = true;
    status = 'Starting light-to-LED automation...';
    timer = setInterval(() => {
      void tick();
    }, 220);
    await tick();
...
<div class="automation">
  <div class="row">
    <label for="light-svc">Light sensor</label>
    <select id="light-svc" bind:value={selectedLightIdx} disabled={running || lightServices.length === 0}>
      {#each lightServices as service, idx}
        <option value={idx}>
  ...
  <div class="actions">
    <button on:click={startAutomation} disabled={running || lightServices.length === 0 || ledServices.length === 0}>
      Start
    </button>
    <button on:click={stopAutomation} disabled={!running}>Stop</button>
  </div>

  <p class="reading">Latest light reading: {latestReading}</p>
