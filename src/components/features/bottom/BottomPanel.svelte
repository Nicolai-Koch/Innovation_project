...
<div
  ...
  {#if $devices.isInputAssigned}
    ...
    <div class="relative w-full h-full">
      ...
      <div class="absolute right-0 bottom-0 h-full w-45 flex flex-col justify-between">
        <button
          type="button"
          class="flex flex-row pl-4 justify-center cursor-pointer hover:bg-secondary hover:bg-opacity-10 transition bg-transparent border-0 p-0 w-full"
  ...
  {:else}
    <div class="h-full w-full flex items-center justify-center bg-white">
      <div class="text-sm text-gray-500">Live graph venter på input-micro:bit</div>
    </div>
  {/if}
</div>
